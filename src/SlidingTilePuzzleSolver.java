...
public class SlidingTilePuzzleSolver {
    ...
    public static ArrayList<SlidingTilePuzzle> uniformCostSearch(SlidingTilePuzzle start) {
        ...
        back.put(start, null);    //start has no backpointer
...
//                System.out.println("Generated: " + numGenerated);
//                System.out.println("Expanded: " + numExpanded);
                return path;
    ...
    public static ArrayList<SlidingTilePuzzle> AStarSearch(SlidingTilePuzzle start, SlidingTilePuzzleHeuristic h) {
        ...
        numExpanded = 0;
        numGenerated = 0;
        pq.offer(start, 0 + h.h(start)); //start first added to the PQ
        generatedSet.put(start, 0 + h.h(start));
        back.put(start, null);    //start has no backpointer
...
//                System.out.println("Expanded: " + numExpanded);
            ...
            for (SlidingTilePuzzle successor : neighborList)
            {
                ...
                {
                    generatedSet.replace(successor, priority + h.h(successor));
                    back.replace(successor, leastCost);
                }
            }
        }
